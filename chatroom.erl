...
do_register(State, Ref, ClientPID, ClientNick) ->
    ...
    % State.
    OldRegistrations = State#chat_st.registrations,
    NewRegistrations = maps:put(ClientPID, ClientNick, OldRegistrations),
...
do_propegate_message(State, Ref, ClientPID, Message) ->
    ...
    Recipients = lists:filter(fun(RecipientPID) -> not (RecipientPID == ClientPID) end, maps:keys(Registrations)),
    lists:map(
        fun(RecipientPID) -> 
            RecipientPID ! {request, self(), Ref, {incoming_msg, SenderNickname, State#chat_st.name, Message}}
        end,
        Recipients
    ),
    ClientPID ! {self(), Ref, ack_msg},
    UpdatedHistory = State#chat_st{history = State#chat_st.history ++ [{SenderNickname, Message}]},
    UpdatedHistory.
