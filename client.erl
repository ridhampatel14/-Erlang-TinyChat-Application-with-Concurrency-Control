...
do_join(State, Ref, ChatName) ->
    ...
    % {{dummy_target, dummy_response}, State}.
	ConnectionChannels = State#cl_st.con_ch,
    case maps:is_key(ChatName, ConnectionChannels) of
        false -> 
            Server = whereis(server),
            Server ! {self(), Ref, join, ChatName},
            receive
                {From, _Ref, connect, History} ->
                    NewConnectionChannels = maps:put(ChatName, From, ConnectionChannels),
                    {History, State#cl_st{con_ch = NewConnectionChannels}}
            end;
        true -> 
            {err, State}
    end.
...
do_leave(State, Ref, ChatName) ->
    ...
    % {{dummy_target, dummy_response}, State}.
	ConnectionChannels = State#cl_st.con_ch,
    case maps:find(ChatName, ConnectionChannels) of
        error -> 
            {err, State};
        {ok, _ChatPID} -> 
            Server = whereis(server),
            ...
            receive 
                {_From, Ref, ack_leave} -> 
                    NewConnectionChannels = maps:remove(ChatName, ConnectionChannels),
                    {ok, State#cl_st{con_ch = NewConnectionChannels}}
            end
    end.
...
do_msg_send(State, Ref, ChatName, Message) ->
    % io:format("client:do_new_nick(...): IMPLEMENT ME~n"),
    % {{dummy_target, dummy_response}, State}.
	ChatroomProcess = maps:get(ChatName, State#cl_st.con_ch),
    ChatroomProcess ! {self(), Ref, message, Message},
    receive
        {ChatroomProcess, Ref, ack_msg} ->
